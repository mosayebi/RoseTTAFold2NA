...
class IterativeSimulator(nn.Module):
    def __init__(
        ...
        if n_ref_block > 0:
            self.str_refiner = Str2Str(d_msa=d_msa, d_pair=d_pair,
                                       d_state=SE3_ref_param['l0_in_features'],
                                       SE3_param=SE3_ref_param, # use alt params
                                       rbf_sigma=rbf_sigma,
                                       p_drop=p_drop,
                                       nextra_l0=6*NTOTALDOFS,
                                       nextra_l1=9 )
    ...
    def forward(self, seq_unmasked, msa, msa_full, pair, xyz, state, idx, use_checkpoint=False):
        ...
        #cycles = np.random.randint(self.n_ref_block)
        if (not xyz.requires_grad):
            ncycles = self.n_ref_block
        else:
            ncycles = np.random.randint(1,self.n_ref_block)

        with torch.no_grad():
            for i_m in range(ncycles-1):
                dbonddxyz, dbonddalpha = calc_BB_bond_geom_grads(
                    seq_unmasked, idx, xyz.detach(), alpha.detach(),
                ...
                dhbdxyz, dhbdalpha = calc_hb_grads(
                    seq_unmasked, xyz.detach(), alpha.detach(), 
                    self.compute_allatom_coords,
                    self.aamask, 
                    self.hbtypes, 
                ...
                extra_l1 = torch.cat((dbonddxyz[0],dljdxyz[0],dhbdxyz[0]), dim=1)
                extra_l0 = torch.cat((
                    dbonddalpha.reshape(1,-1,2*NTOTALDOFS),
                    dljdalpha.reshape(1,-1,2*NTOTALDOFS),
                    dhbdalpha.reshape(1,-1,2*NTOTALDOFS)
                ), dim=2)
